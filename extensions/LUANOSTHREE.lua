...
zg=sgs.CreateTriggerSkill{
	...
	if not event==sgs.TurnStart then return false end
		...
		if (player:objectName()==dengshizai:objectName()) then return end
		if (dengshizai and dengshizai:faceUp()) then
		if (room:askForSkillInvoke(dengshizai,self:objectName())==true) then 
		    room:playSkillEffect("zhenggong")
		    player:gainMark("@zhenggong")
			dengshizai:turnOver()
			dengshizai:play()			
			player:loseMark("@zhenggong")
			player:play()			
			return true end
		end	
	end,
...
fq=sgs.CreateTriggerSkill{
	...
	if  (player:getPhase()==sgs.Player_Play) then
		...
		if (room:askForSkillInvoke(player,self:objectName())~=true) then return false end 
		room:setPlayerFlag(player,"fq")		
		log.type ="#fq"	
		room:sendLog(log)		
		return true		
	elseif (player:getPhase()==sgs.Player_Finish) then	  
		if(not player:hasFlag("fq")) then return  end
		if(room:askForDiscard(player,self:objectName(),1,false,false)) then --return end
			local target = room:askForPlayerChosen(player, room:getOtherPlayers(player), "fangquan")
			local name=target:getGeneralName()
			 if (name == "zhugeliang" or name == "shenzhugeliang" or name == "wolong") then
...
td=sgs.CreateTriggerSkill{
	...
	on_trigger = function(self,event,player,data)
		...
		if(room:askForDiscard(player,self:objectName(),1,false,false)) then 
		player:turnOver()
...
jsvs=sgs.CreateViewAsSkill{
name="jsvs",
n=0,
view_as=function(self, cards)	
	if #cards==0 then			
		local js_card=sgs.Sanguosha:cloneCard("analeptic",sgs.Card_NoSuit, 0)			
		js_card:setSkillName(self:objectName())
		return js_card end	
end,
enabled_at_play=function()
	return sgs.Self:faceUp() 	and not sgs.Self:hasFlag("used")
end,
enabled_at_response=function(self,pattern)	
	 return  (pattern=="analeptic")  and (not player:faceUp())  and (not sgs.Self:hasFlag("used"))
end
}

js=sgs.CreateTriggerSkill{
	name="js",
	events={sgs.CardUsed,sgs. Damaged,sgs.PhaseChange},
	view_as_skill=jsvs,
	priority=3,
	can_trigger=function()
	return true
	...
	if (event==sgs.CardUsed)  then
			local use=data:toCardUse()
			local card = use.card
			if (skillowner:getPhase()==sgs.Player_Play) then
				if skillowner:hasFlag("used") and card:getSkillName()=="jsvs" then return true end		
				if card:getSkillName()=="jsvs" then
				room:playSkillEffect("jiushi", 2)
				room:setPlayerFlag(skillowner,"used")
				skillowner:turnOver()
				return false end
			
			elseif(skillowner:getPhase()==sgs.Player_NotActive) then
				if skillowner:hasFlag("used") and card:getSkillName()=="jsvs" then return true end
				if card:getSkillName()=="jsvs" then
				room:playSkillEffect("jiushi", 1)
				room:setPlayerFlag(skillowner,"used")
				skillowner:turnOver()
				return false end
			end		
	elseif (event==sgs.Damaged) 
	and player:hasSkill("js")
	--and (skillowner:getPhase()==sgs.Player_NotActive) 
	and (not player:faceUp()) then
		if (room:askForSkillInvoke(player,self:objectName())~=true) then return false end
		room:playSkillEffect("jiushi", 3)
		skillowner:turnOver()
	elseif (event==sgs.PhaseChange) and (skillowner:getPhase()==sgs.Player_Finish) then
	     room:setPlayerFlag(skillowner,"-used")
	end
	return false
	end
...
doudizhu = sgs.General(extension, "doudizhu", "qun",3)
doudizhu:addSkill(dizhu)
doudizhu:addSkill(dubo)
...
--争功 放权
dsz = sgs.General(extension, "dsz", "qun",4)
dsz:addSkill(zg)
dsz:addSkill(fq)
dsz:addSkill(td)
dsz:addSkill(js)
