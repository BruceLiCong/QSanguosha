...
bawang_card=sgs.CreateSkillCard{
...
filter=function(self,targets,to_select)
	...
	if to_select:hasFlag("bawang_target") then return false end

	return sgs.Self:canSlash(to_select,true)
end,

on_effect=function(self,effect)
	effect.to:getRoom():setPlayerFlag(effect.to,"bawang_target")
end

}

bawang_viewas=sgs.CreateViewAsSkill{
	name = "bawang_viewas",
	n = 0,
	view_filter = function()
		return false
	...
	enabled_at_response=function()
		return sgs.Self:getPattern()=="#bawang_card"
	end
}

bawang=sgs.CreateTriggerSkill{
	name="bawang",
	view_as_skill=bawang_viewas,
	...
	on_trigger=function(self,event,player,data)
		...
		if isGood then
			...
			room:setPlayerFlag(effect.to,"bawang_target")
			room:askForUseCard(player,"#bawang_card","#bawang_card:"..effect.to:objectName())
			...
			for _,aplayer in sgs.qlist(players) do
				if aplayer:hasFlag("bawang_target") then
					room:setPlayerFlag(aplayer,"-bawang_target")
...
sunce=sgs.General(extension,"sunce","wu")
sunce:addSkill(bawang)
...
sgs.LoadTranslationTable{
	...
	["sunce"]="孙策",
	["bawang"]="霸王",
	[":bawang"]="你的杀被闪避时，你可以与目标拼点；若你赢，则视为你对包含目标在内的最多X名角色使用了杀。x为你已损失的体力值。",
	["#bawang_card"]="你可以另外指定x-1名角色，x为你已损失的体力值",
